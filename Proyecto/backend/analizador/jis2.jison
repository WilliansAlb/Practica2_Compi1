...
wis [W][i][s][o][n]
lexi [L][e][x]
term [T][e][r][m][i][n][a][l]
nterm [N][o]"_"[T][e][r][m][i][n][a][l]
sy [S][y][n][t][a][x]
in [I][n][i][t][i][a][l]"_"[S][i][m]
%x string
%x coment
%x coment2
%%
...
{espacio}             /*solo ignora*/
<<EOF>>               return 'EOF'
.                     {temp_errores.push(instruccionesAPI.nuevoError(yytext,yylloc.first_line,yylloc.first_column,'lexico'));}
...
expressions:
    bloque_analizar  EOF { if (temp_errores.length > 0){ envio = instruccionesAPI.nuevaListaTerms('','','true',temp_errores); temp_errores=[]; return envio} return $1;}
;
