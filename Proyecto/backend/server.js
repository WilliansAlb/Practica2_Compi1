...
const generador = require('./analizador/jis2');
...
app.post('/analizar', function (req, res, next) {
    ...
    if (datos.errores==='false'){
        ...
        if (errores.length==0){
            ...
            res.status(200).json({ correcto: "todo correcto" });
        } else {
            escr.escrituraAPI.escribir(datos);
            res.status(200).json({ errores: errores });
        }
    } else {
        for (let dato of datos.errando){
            errores.push("ERROR "+dato.tipo+": "+dato.parte+" en la linea "+dato.linea+" en la columna "+dato.columna);
        }
        res.status(200).json({ errores: errores });
    }
});
...
app.post('/entrada', function (req, res, next) {
    var texto = req.body.text;
    var textoen = req.body.entrada;
    const generador2 = require('./creados/' + texto);
    var ast = generador2.parse(textoen);
    var str = JSON.stringify(ast, null, 2);
    fs.writeFileSync('./analizador/gen.json', str);
    var datos = JSON.parse(str);
